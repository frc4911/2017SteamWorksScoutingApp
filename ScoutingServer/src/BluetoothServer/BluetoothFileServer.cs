...
namespace Org.USFirst.Frc.Team4911.BluetoothServer
{
    ...
    public class BluetoothFileServer : IDisposable
    {
        ...
        public BluetoothFileServer(Action<string> handler, string root, TraceListener traceListener = null)
        {
            ...
            this.root = root;
            this.traceListener = traceListener ?? new ConsoleTraceListener();
        }
        ...
        public void Listen()
        {
            ...
            this.listener = new BluetoothListener(new Guid("2d31ac7d-0d4a-48dd-8136-2f6a9b71a3f4"));
            this.listener.Start();
            this.listener.BeginAcceptBluetoothClient(this.BluetoothListenerAcceptClientCallback, listener);
        }
        ...
        private void BluetoothListenerAcceptClientCallback(IAsyncResult result)
        {
            using (var client = this.GetClient(result))
            {
                ...
                MessageHandler handler = new MessageHandler(root, stream, this.traceListener);
        ...
        {
            if (result == null || result.AsyncState == null || !(result.AsyncState is BluetoothListener))
            {
                throw new ArgumentNullException(nameof(result));
            }

            var blueToothListener = (BluetoothListener)result.AsyncState;
